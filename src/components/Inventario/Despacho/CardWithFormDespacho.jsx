...
import { LimpiarSecuenciales, registerDespachoRequest, secuencialDesdeRequest, secuencialHastaRequest } from "@/services/inventario";
...
export const CardWithFormDespacho = () => {
  ...
  // const fetchSecuencialDesde = useCallback(async (token) => {
  //   try {
  //     const response = await secuencialDesdeRequest(token);
  //     if (response && response.error === 0) {
  //       // ✅ solo actualiza si el valor está vacío o si es diferente
  //       setSecuencialSalida((prev) => {
  //         if (!prev || prev !== response.secuencial) return response.secuencial;
  //         return prev;
  //       });
  //     }
  //   } catch (error) {
  //     console.error("Error fetching secuencial desde:", error);
  //   }
  // }, []);
  ...
  // const fetchSecuencialHasta = useCallback(async (token) => {
  //   try {
  //     const response = await secuencialHastaRequest(token);
  //     if (response && response.error === 0) {
  //       // ✅ solo actualiza si el valor está vacío o si es diferente
  //       setSecuencialDestino((prev) => {
  //         if (!prev || prev !== response.secuencial) return response.secuencial;
  //         return prev;
  //       });
  //     }
  //   } catch (error) {
  //     console.error("Error fetching secuencial hasta:", error);
  //   }
  // }, []);
  ...
  const handleRegistrar = async () => {
    ...
    const requestDespacho = {
      ...
      try {
        const responseDespacho = await registerDespachoRequest(requestDespacho, token);
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Despacho de Inventario</CardTitle>
        <CardDescription>
          En esta opción se puede gestionar el despacho de inventario de cedulas a todas las oficinas de la corporacion .
        </CardDescription>
      </CardHeader>
