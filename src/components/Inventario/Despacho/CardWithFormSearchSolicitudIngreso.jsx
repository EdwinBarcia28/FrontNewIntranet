...
import { toast } from "react-toastify";
import { searchSolicitudDespachoGeneralRequest, solicitudDespachoGeneralRequest } from "@/services/inventario";
import { useAuthStore } from "@/store/auth";
...
export const CardWithFormSearchSolicitudIngreso = () => {
  ...
  const fetchDataSolicitud = useCallback(async () => {
    try {
      const responseSolicitud = await solicitudDespachoGeneralRequest(token);
  ...
  const handleBuscar = async (event) => {
    ...
    const request = {
      ...
      idOficina: Number(filterSolicitudDespacho) === 3 ? Number(IdOficinaDestino) : null,
      usuario: dataUser?.nombreUsuario ?? "Desconocido",
    };

    try {
      const response = await searchSolicitudDespachoGeneralRequest(request, token);
  ...
  return (
    <Card className="w-full px-4 md:px-10 max-w-[1400px] mx-auto">
      <CardHeader>
        <CardTitle>Consulta de ingresos de stock</CardTitle>
        <CardDescription>
          En esta opción podra consultar los solicitudes de despacho registradas de los solicitantes de stock en todas las oficinas
          , aqui se podra filtrar por el criterio de busqueda.
        </CardDescription>
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-12 gap-6 items-end">
          <div className="md:col-span-3 flex flex-col space-y-1.5">
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="3">Oficina de pedido</SelectItem>
              </SelectContent>
            </Select>
          </div>
          ...
          <div className="md:col-span-7 flex flex-col space-y-1.5">
            ...
            {Number(filterSolicitudDespacho) === 3 && (
              <div className="flex gap-4">
