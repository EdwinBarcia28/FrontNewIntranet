...
import { DialogAnularDespacho } from "./DialogAnularDespacho";
import { anularDespachoRequest, aprobarDespachoRequest, imprimirEtiqueta } from "@/services/inventario";
import { DialogEnviarDespacho } from "./DialogEnviarDespacho";


export function DataTableSearchDespacho({data , onRefresh }) {
  const { dataUser,  token } = useAuthStore();

  const [openDialog, setOpenDialog] = useState(false);
  const [selectedDespachoId, setSelectedDespachoId] = useState(null);
  const [secuencialDesde, setSecuencialDesde] = useState(null);
  const [secuencialHasta, setSecuencialHasta] = useState(null);
  ...
  const columns = [
        ...
        return (
          ...
          <div className="flex justify-center items-center gap-2">
            {data.estado === "Pendiente" && (
              <>
                <DialogEnviarDespacho handleClickEnviarDespacho={handleEnviar} />

                <DialogAnularDespacho handleClickAnularDespacho={handleAnular} />
    ...
    {
      accessorKey: "oficinaSalida",
      headerLabel: "Oficina de Salida",
      header: ({ column }) => {
        return (
          <Button
            variant="ghost"
      ...
      cell: ({ row }) => <div>{row.getValue("oficinaSalida")}</div>,
    },
    {
      accessorKey: "oficinaDestino",
      headerLabel: "Oficina de Destino",
      header: ({ column }) => {
        return (
          <Button
            variant="ghost"
      ...
      cell: ({ row }) => <div>{row.getValue("oficinaDestino")}</div>,
    },
    {
      accessorKey: "cantidadDesde",
      headerLabel: "Secuencial Desde",
      header: ({ column }) => {
        return (
          <Button
            variant="ghost"
      ...
      cell: ({ row }) => <div>{row.getValue("cantidadDesde")}</div>,
    },
    {
      accessorKey: "cantidadHasta",
      headerLabel: "Secuencial Hasta",
      header: ({ column }) => {
        return (
          <Button
            variant="ghost"
      ...
      cell: ({ row }) => <div>{row.getValue("cantidadHasta")}</div>,
    },
    {
...
DataTableSearchDespacho.propTypes = {
