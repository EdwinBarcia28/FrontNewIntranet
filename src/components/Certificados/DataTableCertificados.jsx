...
import { validarCertificadoRequest } from "@/services/certificado";
import { DialogAddObservacionesCertificado } from "./DialogAddObservacionesCertificado";
import { DialogAddObservacionesCheckCertificado } from "./DialogAddObservacionesCheckCertificado";
// import { toast } from "react-toastify";
...
export function DataTableCertificados({ data, onRefresh }) {
  //
  ...
  const columns = [
    {
      ...
      cell: ({ row }) => {
        ...
        const handleCheck = async (id, tipo, parentesco, observacion) => {
          try {
            setGlobalLoading(true, `Validando certificado #${data.idSolicitud} con C.I Solicitante:${data.cedulaSolicitante}`);

            const payload = {
              idSolicitud: Number(id),
              tipo: tipo,
              estadoNuevo: 2,
              mensajeMotivo: observacion,
              operador: dataUser ? dataUser.nombreUsuario : "Desconocido",
              parentesco: parentesco
            };

            try {
              const res = await validarCertificadoRequest(payload, token);

              if (res.success === true) {
                if (onRefresh)
                  onRefresh();
                toast.success(res.message || "Se mando ");
              } else {
                toast.error(res.message || "Error al anular solicitud");
              }
            } catch (error) {
        ...
        const handleAnular = async (id, tipo, parentesco, observacion) => {
          try {
            setGlobalLoading(true, `Validando certificado #${data.idSolicitud} con C.I Solicitante:${data.cedulaSolicitante}`);
            ...
            <div className="flex justify-center items-center gap-2">
              {data.estado === "PENDIENTE" && (
                <>
                  ...
                  {/* <DialogAddObservacionesCheckCertificado handleClickObservacionesCertificado={(observacion) => handleCheck(data.idSolicitud,data.tipoCertificado,data.parentesco,observacion)} /> */}
                  <DialogAddObservacionesCheckCertificado tipoCertificado={data.tipoCertificado}
                    handleClickObservacionesCertificado={(observacion, parentesco) =>
                      handleCheck(data.idSolicitud, data.tipoCertificado, parentesco, observacion)
                    }
                  />

                  <DialogAddObservacionesCertificado handleClickObservacionesCertificado={(observacion) => handleAnular(data.idSolicitud, data.tipoCertificado, data.parentesco, observacion)} />
                </>
              )}
            </div>
          </>
...
DataTableCertificados.propTypes = {
